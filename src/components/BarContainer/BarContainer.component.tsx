...
export const BarContainer: React.FC<BarContainerProps> = props => {
    ...
    const { deleteBar, duplicateBar, alterRepBefore, alterRepAfter, song: { voices } } = useContext(SongContext);
    const bar = props.bar
    ...
    const handleClose = (method: string) => {
        ...
        if (method === "alterRepBefore") {
            alterRepBefore(index);
        }
        if (method === "alterRepAfter") {
            alterRepAfter(index);
        }
        setAnchorEl(null);
    };
                ...
                <Grid item xs={12} role="row">
                    <Grid container style={{ height: "auto" }} role="grid" aria-label="barline before the bar">
                        ...
                        <Grid item xs={centerDivSize} role="gridcell" aria-label="the bar">
                            <Bar height={props.height || 120} repBefore={voices[0].bars[props.barNumber].repBefore} repAfter={voices[0].bars[props.barNumber].repAfter} house={voices[0].bars[props.barNumber].house} chordsAndNotes={bar.chordsAndNotes} barLineAfter={props.barLineAfter} />                        </Grid>
                    </Grid>
                </Grid>
                        ...
                        <Grid item xs={10} role="gridcell" style={{ display: props.masterSheet ? "block" : "none" }}>
                            <Box display="flex" flexGrow={1}>
                                ...
                                <Menu id="menuBar" anchorEl={anchorEl} keepMounted open={Boolean(anchorEl)} onClose={handleClose} role="menu">
                                    ...
                                    <MenuItem onClick={() => handleClose("alterRepBefore")}>Legg til repetisjon foran </MenuItem>
                                    <MenuItem onClick={() => handleClose("alterRepAfter")}>Legg til repetisjon bak </MenuItem>
                                </Menu>
                            </Box>
                        </Grid>
                        <Grid item xs={1}></Grid>
